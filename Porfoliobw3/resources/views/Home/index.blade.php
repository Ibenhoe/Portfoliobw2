       ...
       <div style="width: 100%; max-width: 800px; margin: 20px auto; padding: 20px; background-color: #f9f9f9; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
             ...
             @endif
                  ...
                  @foreach($newsItems as $item)
                       <div style="width: 300px; margin-bottom: 20px; border: 1px solid #ddd; border-radius: 5px; padding: 10px;">
                            <h2 style="font-size: 1.3em; margin-bottom: 10px;">{{ $item->title }}</h2>
                           @if ($item->image)
                                <img src="{{ asset('storage/' . $item->image) }}" alt="News Image" style="max-width: 100%; height: auto; margin-bottom: 10px;">
                           @endif
